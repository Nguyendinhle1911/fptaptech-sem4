...
const Appointment = () => {
    ...
    useEffect(() => {
        if (formData.department) {
            axios.get(`http://localhost:8081/api/v1/departments/search?department_id=${formData.department}`)
                .then(response => {
            ...
            axios.get(`http://localhost:8081/api/v1/departments/${formData.department}/doctors`)
                .then(response => {
    ...
    useEffect(() => {
        if (formData.doctor) {
            axios.get(`http://localhost:8081/api/v1/doctors/search?doctor_id=${formData.doctor}`)
                .then(response => {
                    if (response.data.length > 0) {
    ...
    useEffect(() => {
        if (formData.doctor && formData.date) {
            axios.get(`http://localhost:8081/api/v1/appointments/${formData.doctor}/slots`)
                .then(response => {
            ...
            axios.get(`http://localhost:8081/api/v1/appointments/check-locked-slots?doctorId=${formData.doctor}&date=${formData.date}`)
                .then(response => {
    ...
    const submitAppointment = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:8081/api/v1/appointments/insert', dataToSend);
    ...
    const sendEmailFormRegister = (doctorName, departmentName, medicalDay, patientEmail, patientName, timeSlot) => {
        ...
        return axios.post('http://localhost:8081/api/v1/appointments/send-email', {
            doctorName,
            departmentName,
            medicalDay,
            patientEmail,
    ...
    const sendEmailToDoctor = (doctorName, departmentName, medicalDay, doctorEmail, patientName, timeSlot) => {
        ...
        return axios.post('http://localhost:8081/api/v1/appointments/send-email-doctor', {
            doctorName,
            departmentName,
            medicalDay,
            doctorEmail,
                                ...
                                <>
                                    <div className={`form-section2 ${animationClass}`}>
                                        ...
                                        <div className="button-grp">
                                            <button onClick={handlePrevStep} disabled={isAnimating} className="prev-step">Previous Step
                                            </button>
                                            <button onClick={handleNextStep} className="next-step"
