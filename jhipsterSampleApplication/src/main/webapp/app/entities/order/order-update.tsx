        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField
                label={translate('jhipsterSampleApplicationApp.order.orderDate')}
                id="order-orderDate"
                name="orderDate"
                data-cy="orderDate"
                type="datetime-local"
              ...
              />
              <ValidatedField
                label={translate('jhipsterSampleApplicationApp.order.status')}
                id="order-status"
                name="status"
                data-cy="status"
                type="select"
              >
                {orderStatusValues.map(orderStatus => (
                  <option value={orderStatus} key={orderStatus}>
                    {translate(`jhipsterSampleApplicationApp.OrderStatus.${orderStatus}`)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
              ...
              <ValidatedField
                ...
                name="customer"
                data-cy="customer"
                label={translate('jhipsterSampleApplicationApp.order.customer')}
                type="select"
              >
                <option value="" key="0" />
                {customers
